...
export async function scrollDown(page: Page, element: ElementHandle<Element>, amount: number) {
    await page.evaluate((element : any, amount : number) => {
        element.scrollBy(0, amount);
    }, element, amount);
}

export async function queryByText(page: Page, parent: ElementHandle<Element>, text: string) {
...
export async function getTextContent(page: Page, element: ElementHandle<Element>) {
...
export async function getPropValue(page: Page, handle: ElementHandle<Element>, name: string) {
    if (handle) {
