...
export async function findContactLinks(page: Page): Promise<string[] | null> {
    ...
    const absoluteURLs: string[] = rawContactLinks.map((linkHref) => convertToAbsoluteURL(page.url(), linkHref));
...
function convertToAbsoluteURL(currentPageURL: string, linkHref: string): string {
    ...
    const isAbsoluteURL = linkHref.startsWith('http://') || linkHref.startsWith('https://');
...
function removeMailTo(links: string[]): string[] {
    ...
    return links.filter(item => !wordsToRemove.some(word => item.includes(word)));
}